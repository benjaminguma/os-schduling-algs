...
function RoundRobin({ read, queue, wasteTime, story, tq }) {
	...
	function hasLastValid(slave, master) {
		// checks if master(ready queue) has remaining bt for slave queues last value
	...
	function incomingAt(time2, time) {
		...
		if (read.some((p) => p.at !== undefined)) {
			...
			if (!modifiedQueue && hasLastValid(queue2, read)) queue = [...queue, queue2[queue2.length - 1]];
	...
	for (let i = 0; ; i++) {
		...
		wasteTime && addToStory({ pid: 'id', time: wasteTime, start: i - wasteTime, b: 'N/A' }, story);
