...
function collectdata(dataPack) {
  ...
  for (let i = 0; i < dataPack.length; i += 2) {
    const at = dataPack[i].value,
      bt = dataPack[i + 1].value;
    if ((!at && at !== 0) || !bt || isNaN(at))
      throw new Error("values are incomplete");
    at = Math.round(at);
    bt = Math.round(bt);
    read.push({ at, bt });
  }
  ...
  <div class="row">
         ...
         <div class="u-center summry cp2">
            <div class=" heading--small mb-1 p-1">a.t.a.t</div>
              <div class=" heading--small mb-1 p-1 "> ${atat.toPrecision(
                5
              )}ms</div>
             </div>
         </div>
      <div class="col-1-4 u-center">
         <div class="u-center summry cp4">
              ...
              <div class=" heading--small mb-1 p-1 "> ${awt.toPrecision(
                5
              )}ms</div>
             </div>
      </div>
...
window.onload = () => {
  ...
  form.onsubmit = function (e) {
    ...
    if (!specs[0].value) {
      handleError(
        "#caution",
        "error:specify how many processes you want to continue"
      );
      return;
    }
  ...
  form2.onclick = function (e) {
    ...
    try {
      ...
      $2(".j").forEach((e) =>
        authenticate.bind(e, null, e.value === "" || isNaN(e.value))()
      );
    ...
    dataRR = JSON.parse(
      JSON.stringify({ ...data, tq: Number(specs[1].value) })
    );
    ...
    if (data) {
      if (boxes.every(({ checked }) => !checked)) {
        handleError(
          "#caution",
          "error:please specify an algorithm from the options above"
        );
        return;
      }
      if (boxes[0].checked) {
      ...
      boxes[3].checked &&
        BuildContainer("#test", SRTF(dataSRTF), null, bigContainer, data);
      boxes[2].checked &&
        BuildContainer("#test1", SJF(dataSJF), null, bigContainer, data);
      boxes[1].checked &&
        BuildContainer("#test2", FCFS(dataFCFS), null, bigContainer, data);
    }
  };
};
